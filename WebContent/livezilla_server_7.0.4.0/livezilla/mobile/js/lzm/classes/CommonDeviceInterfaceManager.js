            ...
            switch(_msgData.tab){
                case 'ticket-reply':
                    if(!($('#ticket-details-container').length)){
                        SelectView('tickets');
                        ChatTicketClass.__ShowTicket(_msgData.id.ticket.id, true, '', '', '');
                        $(document).find('#reply-ticket-details').click();
                        $(document).find('#reply-placeholder-tab-1').click();
                        // //$(document).find('#add-attachment').click();
                        ticketReplyAddAttachmentFile(_msgData.id);
                    }else {
                        // //$(document).find('#add-attachment').click();
                        ticketReplyAddAttachmentFile({screenshot:true, data:_msgData.id});
                    }
                    break;
                case 'knowledgebase':
                    SelectView('qrd');
                    KnowledgebaseUI.HandleResourceClickEvents(_msgData.id);
                    addQrd(3);
                    break;
                case 'chat':
                    TaskBarManager.ClickElement(_msgData.id);
                    addQrdToChat(3);
                    break;
            }
        });
